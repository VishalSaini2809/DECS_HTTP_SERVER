...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        string a = argv[i];
        if (a == "--url") cfg.server_url = argv[++i];
        else if (a == "--port") cfg.port = stoi(argv[++i]);
        else if (a == "--clients") cfg.clients = stoi(argv[++i]);
        else if (a == "--dur") cfg.duration_sec = stoi(argv[++i]);
        else if (a == "--keyspace") cfg.keyspace = stoi(argv[++i]);
        else if (a == "--popular") cfg.popular = stoi(argv[++i]);
        else if (a == "--workload") {
            string w = argv[++i];
            if (w == "put-all") cfg.workload = PUT_ALL;
            else if (w == "get-all") cfg.workload = GET_ALL;
            else if (w == "get-popular") cfg.workload = GET_POPULAR;
            else cfg.workload = MIXED;
        }
    }
